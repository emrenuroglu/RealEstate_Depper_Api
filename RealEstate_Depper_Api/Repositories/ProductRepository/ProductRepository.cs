...
using RealEstate_Depper_Api.Dtos.ProductDetailDtos;
using RealEstate_Depper_Api.Dtos.ProductDtos;
using RealEstate_Depper_Api.Models.DapperContext;

namespace RealEstate_Depper_Api.Repositories.ProductRepository
{
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task<List<ResultpProductWithCategoryDto>> GetAllProductWithCategoryAsync()
        {
            ...
            using (var connection = _context.CreateConnection())
            {
                var values = await connection.QueryAsync<ResultpProductWithCategoryDto>(query);
                return values.ToList();
            }
        ...
        public async Task<List<ResultpProductWithCategoryDto>> GetProductByDealOfTheDayTrueWithCategoryAsync()
        {
            string query = "Select ProductID,Title,Price,City,District,CategoryName,CoverImage,Type,Address,DealOfTheday From Product " +
                "inner join Category on Product.ProductCategory=Category.CategoryID where DealOfTheDay=1";
            using (var connection = _context.CreateConnection())
            {
                var values = await connection.QueryAsync<ResultpProductWithCategoryDto>(query);
                return values.ToList();
